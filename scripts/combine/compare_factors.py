...
if False:   #Compile and save the data
    ...
    for plot_num, batch_id in enumerate(np.arange(0,8)):
        ...
        ax.scatter(chb_ref, chb, alpha = 0.1, c = 'blue', s = 0.1)
        chb_biases.append(bias)

    plt.suptitle('Coronal Hole Boundary Distances')
    plt.tight_layout()
    plt.savefig('./plots/chb_corr.png')
    plt.close()
    ...
    for plot_num, batch_id in enumerate(np.arange(0,8)):
        ...
        fs = np.load(f'./data/factor_comparison/fs_{plot_num}.npy')
