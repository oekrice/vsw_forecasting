...
parameter_sources = ["combine_raw", "combine_adjust"]
scale_sources = ["WSA", "combine", "combine_optimised", "reference"]

parameter_set_number = 0   #Can loop through this if necessary

#To copy raw speeds from Hamilton

# scp -r vgjn10@hamilton8.dur.ac.uk:/nobackup/vgjn10/projects/vsw_forecasting/data/raw_speeds/combine_raw* ./data/raw_speeds

for parameter_set_number in [1,2]:
    for a in [0,1]:
        bs = [0,1,2]
        #bs = [0,2,3]
        for b in bs:
            ...
            for i in range(0,8):
                batch_names.append(f'{parameter_source}_{parameter_set_number}_{i}')
            ...
            plt.suptitle(f"{parameter_source}_{scale_source}_{parameter_set_number}")

            plt.tight_layout()
            plt.savefig(f'./plots/combine_plots/{a}_{b}_{parameter_set_number}.png')
