...
def run_model_combined(run_parameters, theta=None, snap_subset=None, iteration=0, output_distributions=False, save_speeds=False, use_old_chb_formula=False):
    ...
    if run_parameters["calculate_huxt"]:
        for si, snap_id in enumerate(snap_subset):
            ...
            ax.plot(times, model_speeds, linewidth=0.2, c = plot_colour)
        ...
        if model_type == 0:
            ax.plot(times_avg, wsa, linewidth = 2, c = plot_colour, label = 'Default WSA Parameters')
        elif model_type == 1:
            ax.plot(times_avg, wsa, linewidth = 2, c = plot_colour, label = 'Optimised WSA Parameters')
        elif model_type == 2:
            ax.plot(times_avg, wsa, linewidth = 2, c = plot_colour, label = 'Neural Net')
...
for iteration_number in range(1000,1001):
    ...
    for model_type in range(3):
        ...
        ax = axs[0, model_type]
        ax.pcolormesh(global_vr, vmin=200, vmax=900)
        ax.set_xticks([])
        ax.set_yticks([])
    ...
    plt.tight_layout()
    plt.savefig(f'./plots/overfit_converge/{set_type}_{iteration_number:05d}.png')
    plt.close()
