...
for a in [0]:
    ...
    for b in bs:
        ...
        for i in range(8):
            ...
            axs[0].plot(crot_numbers, all_rmss, linewidth=1.0)
            axs[1].plot(crot_numbers, all_rs, label = make_nicetitle(i), linewidth=1.0)
            ...
            if not plotted_omni: #The OMNI Reference
                ...
                for ci, crot in enumerate(crot_numbers[:-6]):
                    ...
                    all_rmss[ci] = rms
                    all_rs[ci] = r
                axs[0].plot(crot_numbers, all_rmss, c = 'black', linestyle='dashed',linewidth=1.0)
                axs[1].plot(crot_numbers, all_rs, label = 'Persistence', linestyle='dashed', c = 'black', linewidth=1.0)
                plotted_omni=True
