...
# Path to output directory [finish date will be appended]:
# outputpath = "/Volumes/bmjg46/prs/dumfric/fcast-test1"
outputpath = "/Volumes/bmjg46/prs/dumfric/fcast-test2"

# Desired finish/"forecast" time:
# t_f = "20250407.12"
t_f = "20250401.12"
...
if pregs != []:
    for k, preg in enumerate(pregs):
        ...
        ayplot.mesh(axes[3], np.rad2deg(sft.pg), sft.sg, br2, fmax=50, ylabel='Sine Latitude', title=r'Reconstructed $B_r$ [after smoothing+twisting]')
        axes[3].contour(np.rad2deg(sft.pc), sft.sc, msk1[1:-1,1:-1], [0.5], colors='k', linewidths=0.75)
        axes[3].text(0.1, 0.1, 'Max error: %g' % (np.max(np.abs(br2-br1))), transform=axes[3].transAxes)
        ...
        # - if all is well then output region file [keep track of multiple files per day]:
        #   [for very small regions occasionally the computation of vector potential gives NaN]
        if ( np.max(np.abs(br2-br1)) < 0.1*np.max(np.abs(br1)) ):
