...
# Desired finish time [time needs to be 12:00 for emergence to work properly]:
t_f = "20230101.12"
dtime_f = datetime.datetime.strptime(t_f, "%Y%m%d.%H")
print("Simulation forecast time: t_f = "+t_f)

# Set this to True to ignore observational data that wouldn't have been available [for testing]:
pretend_fcast = True

outputpath += "_s"+t_f+"/"

# Length of spinup in days [needs to be long enough that a full carrington map is available for cr_0]:
dt_spinup = 60
...
# -------------------------------------------------------------------------
# - for testing with past data, turn this off with pretend_fcast=True:
if pretend_fcast:
    cr = cr_0 + 1
    cr_first_incomplete = cr_0+1
else:
...
while (dtime < dtime_newest + datetime.timedelta(days=4.545)) & (not exit_flag):
    ...
    if dtime + datetime.timedelta(days=4.545) > dtime_newest:
        ...
        delta_dtime = dtime_newest - dtime.datetime
        dtime = dtime_newest
        exit_flag = True
