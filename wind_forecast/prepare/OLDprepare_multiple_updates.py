"""
    PREPARE_MULTIPLE_UPDATES.PY - prepare sequence of updates of existing DUMFRIC-FCAST run.

    Dummy initial snapshots are created. (These will need to be replaced before running fortran).

    The purpose of this code is to set up multiple test runs using historic data.
...
initial_inputpath = "/Volumes/bmjg46/prs/dumfric/fcast-repl7_s20230101.12/"
dtime_input = datetime.datetime.strptime(initial_inputpath[-12:-1],"%Y%m%d.%H")

# Frequency to output:
outfreq_days = 7

# Overall end time:
# dtime_overall_end = datetime.datetime(2024, 1, 1, 12)
dtime_overall_end = datetime.datetime(2023, 2, 1, 12)

dtime_f = dtime_input + datetime.timedelta(days=outfreq_days)
inputpath = initial_inputpath

while dtime_f <= dtime_overall_end:
    ...
    try:
        ...
        # make dummy file to allow directories to be set up ahead of time:
        os.system("touch "+outputpath+"b"+t_restart+".nc")

    # ===============================
    inputpath = outputpath
    dtime_f += datetime.timedelta(days=outfreq_days)
