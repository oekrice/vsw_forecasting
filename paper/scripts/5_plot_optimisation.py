...
def run_model_combined(run_parameters, theta=None, snap_subset=None, iteration=0, output_distributions=False, save_speeds=False, use_old_chb_formula=False):
    ...
    omni_fname = './paper/data/shared_data/omni.csv'
    omni_data = []
    ...
    if run_parameters["calculate_huxt"]:
        for si, snap_id in enumerate(snap_subset):
            ...
            if run_parameters["velocity_type"] == "wsa" or run_parameters["velocity_type"] == "wsa_scaled" or run_parameters["velocity_type"] == "wsa_combined" or run_parameters["velocity_type"] == "wsa_scaled_overfit":
                ...
                if theta is not None:
                    ...
                    if si == 0:
                        vr = wf.field_calculations.compute_vr(snap_id, run_name, method="wsa", params = None, doplot=run_parameters["do_plots"], iteration=iteration, huxt_name=run_parameters["run_name"])
                    else:
                        vr = wf.field_calculations.compute_vr(snap_id, run_name, method="wsa", params = None, doplot=False, iteration=iteration, huxt_name=run_parameters["run_name"])

            elif run_parameters["velocity_type"] == "neural_net":
                if run_parameters["verbose"]:
                    print('Using neural net parameters', theta)
                if si == 0:
                    vr = wf.field_calculations.compute_vr(snap_id, run_name, method="neural_net", params = theta, doplot=run_parameters["do_plots"], iteration=iteration, huxt_name=run_parameters["run_name"])
                else:
                    vr = wf.field_calculations.compute_vr(snap_id, run_name, method="neural_net", params = theta, doplot=False, iteration=iteration, huxt_name=run_parameters["run_name"])
            ...
            ax.plot(times, model_speeds, linewidth=0.1, c = plot_colour, alpha = 0.1)
        ...
        if model_type == 0:
            ax.plot(times_avg, wsa, linewidth = 1, c = plot_colour, label = 'Default WSA')
        elif model_type == 1:
            ax.plot(times_avg, wsa, linewidth = 1, c = plot_colour, label = 'Optimised WSA')
        elif model_type == 2:
            ax.plot(times_avg, wsa, linewidth = 1, c = plot_colour, label = 'Neural Net')

        ax.plot(times_avg, omni, linewidth = 1, c = 'black', label = 'OMNI Measurements')
...
def load_directory():
    directory_fname = f'./paper/data/{test_parameters["run_name"]}/log.csv'
...
iteration_number = 1000  #Set to really high so it just does the latest one? Why not?
fig, axs = plt.subplots(2,3, figsize=(fig_width,0.5*fig_width))
...
for model_type in range(3):
    ...
    if model_type == 0:
        ...
        batch_name = f"overfit_{set_type}_{batch_id}"
        velocity_type = "wsa_scaled_overfit"   #To be used for the physics-informed parameter-changing. Just a select few of them. See if there are any mad patterns.
    elif model_type == 2:
        batch_name = f"overfit_{set_type}_net_{batch_id}"
    ...
    if not os.path.exists(f"./paper/data/{test_parameters['run_name']}"):
        os.mkdir(f"./paper/data/{test_parameters['run_name']}")

    #Save a log to let the thing know it's started, for logging purposes
    np.savetxt(f"./paper/data/{test_parameters['run_name']}/start.dat", [n_cores])
    ...
    if iteration_number == -1:
        ...
        if model_type == 2: #Happy to carry on beyond the limit of the WSA optimisation
