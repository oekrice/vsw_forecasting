...
!-----------------------------------------------------------------------
    ...
    subroutine compute_outeqm(brsurf, v1, br, bs, bp) 
        ...
        do m=1,np-1
            do l=1,ns-1
                ...
                if (abs(cml) < 1d-10) then
                    ...
                    ! hc(:,m,l) = hc(:,m,l)/((hc(2,m,l) - hc(1,m,l))/dr + (1-vc(1)/radc(1))*hc(1,m,l))  ! modified to allow v non-zero on lower boundary
        ...
        !$omp parallel private(i,m,k,A)
        !$omp do
        do j=2,ns
            do i=1,nr
                do m=1,np-1
                    A = sum(g(i,m,:)*legs(m,j-1,:))
                    do k=2,np
                        br(i,j,k) = br(i,j,k) + A*trigs(k-1,m)
                    end do
                end do
            end do
        end do
        ...
        do j=2,ns-1
            do i=2,nr
                do m=1,np-1
                    ...
                    do k=2,np
                        bs(i,j,k) = bs(i,j,k) + A*trigs(k-1,m)
                    end do
                end do
            end do
        end do
        ...
        do j=2,ns
            do i=2,nr
                do m=1,np-1
                    A = sum(hc(i,m,:)*legs(m,j-1,:))/sigc(j-1)/dp
                    do k=2,np-1
                        bp(i,j,k) = bp(i,j,k) + A*(trigs(k,m) - trigs(k-1,m))
                    end do
                    bp(i,j,1) = bp(i,j,1) + A*(trigs(1,m) - trigs(np-1,m))
                end do
                bp(i,j,np) = bp(i,j,1)
            end do
        end do
    ...
    subroutine vout(rs, v1, vr)
        ...
        real(d), parameter :: R1 = 2.5_d
        character*(*), parameter :: profile = 'PARKER'
        ! ---
        ...
        if (profile.eq.'PARKER') then
            vr = exp(-2.0_d*RCRIT/exp(rs)) * R1**2
            vr = vr / exp(-2.0_d*RCRIT/R1) / exp(2.0_d*rs)
            vr = vr * v1
        end if

        if (profile.eq.'ANALYTICAL') &
            vr = exp(-rs) * R1 * v1
    ...
    subroutine voutdiff(rs, v1, dvr)
        ...
        character*(*), parameter :: profile = 'PARKER'
        ! ---
        ! Specify r * v_r' function for outflow (here r means radius).
        ! This is (an approximation to) Parker's solar wind solution, 
        ! with critical radius RCRIT.
        ! Parameter v1 is the overall magnitude.
        ! (Could be changed to any reasonable equivalent.)
        ...
        if (profile.eq.'PARKER') then
            dvr = v1 * R1**2 * exp(2.0_d*RCRIT/R1)
